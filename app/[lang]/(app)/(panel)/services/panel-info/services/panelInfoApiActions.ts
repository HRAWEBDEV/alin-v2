...
function getProgramUID(program: Program): string {
 return `${program.id}_${program.systemID}`;
}
...
function getStoreOwnerInfo(ownerInfo: OwnerInfo['value']) {
 ...
 function updateMenusLink(menus: Menu[], parents: Menu[] | null = null) {
  menus.forEach((menu) => {
   ...
   if (!!menu.childs.length) {
    updateMenusLink(
     menu.childs,
     menu.parents ? [...menu.parents, menu] : [menu],
    );
   }
  });
 }
 ...
 ownerInfo.programs.forEach((program) => {
  ...
  ].programs[getProgramUID(program)] = {
   ...program,
...
export { getOwnerInfoApi, getOwnerInfo, getStoreOwnerInfo, getProgramUID };
